...
public class LessThanOrEqualsToPredicate extends AbstractPredicate {
	...
	public <T> Predicate toPredicate(Class<T> clazz, Root<T> root, CriteriaQuery<?> cq, CriteriaBuilder cb) {
		...
		if (dataFieldType == Long.class) return cb.le(path.as(Long.class),((Number) this.getValue()).longValue());
		if (dataFieldType == Integer.class) return cb.le(path.as(Integer.class), ((Number) this.getValue()).intValue());
		if (dataFieldType == Short.class) return cb.le(path.as(Short.class), ((Number) this.getValue()).shortValue());
